...
describe('FareFamilySelection', () => {
  it('renders the section title and every family card', () => {
    render(<FareFamilySelection families={families} selectedFamilyId="light" />);
  ...
  it('marks only the selected family as selected', () => {
    render(<FareFamilySelection families={families} selectedFamilyId="standard" />);
    expect(screen.getByText('Selecionado')).toBeOnTheScreen();
    expect(screen.getAllByText('Selecionar')).toHaveLength(1);
  });

  it('fires onSelectFamily with the pressed family id', () => {
    const onSelectFamily = jest.fn();
    render(
      <FareFamilySelection families={families} selectedFamilyId="light" onSelectFamily={onSelectFamily} />,
    );
    fireEvent.press(screen.getAllByText('Selecionar')[0]);
    expect(onSelectFamily).toHaveBeenCalledWith('standard');
  });

  it('fires onPressDetails when "Ver tarifa" is pressed', () => {
    const onPressDetails = jest.fn();
    render(<FareFamilySelection families={families} onPressDetails={onPressDetails} />);
