...
describe('FlightCard', () => {
  it('renders airline, route, times and duration', async () => {
    await render(<FlightCard {...baseProps} />);
  ...
  it('shows a custom stops label', async () => {
    await render(<FlightCard {...baseProps} stopsLabel="1 parada" />);
    expect(screen.getByText('1 parada')).toBeOnTheScreen();
  });

  it('reflects the selected state', async () => {
    await render(<FlightCard {...baseProps} selected />);
    expect(screen.getByRole('radio')).toHaveProperty('props.accessibilityState.selected', true);
  });

  it('fires onSelect when the card is pressed', async () => {
    const onSelect = jest.fn();
    await render(<FlightCard {...baseProps} onSelect={onSelect} />);
    await fireEvent.press(screen.getByRole('radio'));
    expect(onSelect).toHaveBeenCalledTimes(1);
  });

  it('fires onPressDetails without triggering onSelect', async () => {
    const onSelect = jest.fn();
    const onPressDetails = jest.fn();
    await render(<FlightCard {...baseProps} onSelect={onSelect} onPressDetails={onPressDetails} />);
    await fireEvent.press(screen.getByLabelText('Ver detalhes do voo'));
