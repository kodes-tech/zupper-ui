...
describe('HotelGuestsConfig', () => {
  it('renders the room, adults, children and the apply footer', () => {
    render(<HotelGuestsConfig rooms={[{ adultQty: 1, childAges: [] }]} />);
  ...
  it('renders one age selector per child with the correct label', () => {
    render(<HotelGuestsConfig rooms={[{ adultQty: 1, childAges: [0, 8] }]} />);
  ...
  it('only shows "Remover quarto" when there is more than one room', () => {
    const { rerender } = render(<HotelGuestsConfig rooms={[{ adultQty: 1, childAges: [] }]} />);
  ...
  it('fires the stepper and apply callbacks', () => {
    ...
    fireEvent.press(screen.getByRole('button', { name: 'Aumentar Adultos do quarto 1' }));
