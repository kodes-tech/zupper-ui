...
describe('HotelNearbyCard', () => {
  it('renders the title and the points', () => {
    render(<HotelNearbyCard points={points} />);
  ...
  it('fires onSeeMap only for points with a map link', () => {
    const onSeeMap = jest.fn();
    render(<HotelNearbyCard points={points} onSeeMap={onSeeMap} />);
    fireEvent.press(screen.getByRole('button', { name: 'Ver Parque Ibirapuera no mapa' }));
